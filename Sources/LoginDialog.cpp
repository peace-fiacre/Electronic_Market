               ...
               wxDefaultPosition, wxSize(600, 700))
{
    ...
    m_delivererRadio = new wxRadioButton(panel, ID_DELIVERER_RADIO, "Livreur / Livreur");
    radioSizer->Add(m_delivererRadio, 0);
...
void LoginDialog::OnLogin(wxCommandEvent& event)
{
    ...
    else if(m_delivererRadio->GetValue())
    {
        m_userType = "LIVREUR";
    }
    ...
    else if(m_userType == "LIVREUR")
    {
        if(id == "livreur" && password == "livreur123")
        {
            loginSuccess = true;
            message = "Bienvenue Livreur : " + id;
        }
        else
        {
            message = "Identifiant ou mot de passe incorrect.";
